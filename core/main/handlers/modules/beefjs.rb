      ...
      module Modules
        ...
        module BeEFJS
          ...
          def build_beefjs!(req_host)
            ...
            # @note Load gdrive library only if gDrive feature is enabled
            if config.get("beef.http.gdrive.enable") == true
              beef_js_sub_files << "gdrive.js"
            end
            ...
            # @note Set some gDrive properties
            if config.get('beef.http.gdrive.enable') == true
              hook_session_config['websocket_sec_port']= config.get("beef.http.websocket.secure_port")
            end
